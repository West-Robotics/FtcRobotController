        ...
        score = with (drive.trajectorySequenceBuilder(dropOff.end())) {
            when (prop) {
                PropPositionProcessor.PropPosition.LEFT -> {
                    this.back(11.0)
                        .turn(toRadians(-45.0))
                        .back(12.0, SampleMecanumDrive.getVelocityConstraint(40.0, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH), SampleMecanumDrive.getAccelerationConstraint(10.0))
                }
                PropPositionProcessor.PropPosition.MIDDLE -> {
            ...
            when (side) {
                ...
                when (prop) {
                    // PropPositionProcessor.PropPosition.LEFT -> this.strafeLeft(10.0)
                    PropPositionProcessor.PropPosition.LEFT -> this.strafeLeft(2.0)
                    PropPositionProcessor.PropPosition.MIDDLE -> this.strafeLeft(2.0)
                    PropPositionProcessor.PropPosition.RIGHT -> this.strafeRight(8.0)
                }
