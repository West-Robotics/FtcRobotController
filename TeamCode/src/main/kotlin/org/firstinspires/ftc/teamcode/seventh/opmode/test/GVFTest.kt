...
class GVFTest : LinearOpMode() {
    override fun runOpMode() {
        val path: Path = path {
            hermite {
                label("test")
                start { pos(0.0, 0.0); ang(0.0); v(30.0) }
                end { pos(25.0, 25.0); ang(0.0); v(30.0) }
            }
        }
        ...
        while (opModeIsActive() && !isStopRequested) {
            ...
            val distToEnd = path.paths[0].endPose.position.distanceTo(Vector2d(drive.poseEstimate.x, drive.poseEstimate.y))/3
            val multiplier = if (distToEnd > 1.0) 1.0 else distToEnd
            val correctedRRPose = RRPose2d((rrPose.y*sin(heading) + rrPose.x*cos(heading))*multiplier*0.4,
                    (rrPose.y*cos(heading) - rrPose.x*sin(heading))*multiplier*0.7,
                    0.0)
