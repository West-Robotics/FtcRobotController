...
class OutputSubsystem(hardwareMap: HardwareMap) : Subsystem {
    ...
    fun update(s: RobotState, armAng: Double) {
        outState = when (s) {
            ...
            RobotState.EXTEND   -> OutputState(armAng, -75.0, FINGER_CLOSE, FINGER_CLOSE)
    ...
    override fun write() {
        armLeft.setPosition(outState.arm)  { theta: Double -> theta/180.98 + 1.0 }
        armRight.setPosition(outState.arm) { theta: Double -> theta/180.98 + 1.0 }
        pitch.setPosition(outState.pitch)  { theta: Double -> theta/270.0 + 0.5 }
